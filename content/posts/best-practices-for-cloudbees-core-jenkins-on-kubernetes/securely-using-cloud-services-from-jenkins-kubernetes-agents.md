...
In the second part of this series on [best practices for Jenkins (and CloudBees Core) on Kubernetes](/series/best-practices-for-cloudbees-core-jenkins-on-kubernetes/) we will continue to look at security. In this post we will look at how to reduce security risk of using cloud services from Jenkins Kubernetes agents, similar to how [the previous post in this series](/posts/best-practices-for-cloudbees-core-jenkins-on-kubernetes/core-psp/) showed how Kubernetes Pod Security Policies can be used with Jenkins Kubernetes agents to limit the security risk of Jenkins agent containers.
...
5. Inside the Jenkins Pipeline, use the gcloud SDK to authenticate using the key file from the Kubernetes Secret.
